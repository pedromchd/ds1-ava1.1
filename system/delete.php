...
if (isset($_POST['system'])) {
  ...
  $stmt = $db->prepare('DELETE FROM system WHERE id = ?');
    ...
    <section>
      ...
      <article class="mt-3 flex gap-3">
        <form action="/system/delete.php" method="post">
          <input type="hidden" name="system" value="<?= $row['id'] ?>">
          <input type="submit" value="Delete" class="px-5 py-1 bg-blue-300 cursor-pointer hover:bg-blue-400">
        </form>
        <a href="/system/view.php" class="text-center px-5 py-1 bg-blue-300 hover:bg-blue-400">Cancel</a>
      </article>
    </section>
  </main>
</body>
