...
if (isset($_POST['name'], $_POST['owner'])) {
  ...
  $stmt = $db->prepare('INSERT INTO system (name, owner) VALUES (?, ?)');
  ...
  <main class="flex-grow grid place-items-center">
    <form action="/system/new.php" method="post">
      <fieldset class="w-[25rem] p-5 border-2 border-blue-800 grid gap-2">
        <legend class="px-1 text-xl text-blue-800 font-semibold">New</legend>
        <label for="name">Name:</label>
        <input type="text" name="name" id="name" required class="px-1 py-0.5 border border-blue-500 focus:outline-blue-600">
        <label for="owner">Owner:</label>
        <input type="text" name="owner" id="owner" required class="px-1 py-0.5 border border-blue-500 focus:outline-blue-600">
        <input type="submit" value="Add" class="mt-2 py-1 bg-blue-300 cursor-pointer hover:bg-blue-400">
